...
import { Badge, SignalBadge, SystemBadge } from '@/components/ui/Badge'
...
function classifyIndicator(
  ...
  if (key === 'Bollinger Upper' || key === 'bollinger_bands_upper' ||
    ...
    if (pos > 0.8) return { label: 'Overbought', color: 'warning' }
    if (pos < 0.2) return { label: 'Oversold',   color: 'success' }
    return                 { label: 'Neutral',    color: 'neutral' }
  }
  ...
  // MACD Signal — classify by comparing to MACD Line
  if (key === 'MACD Signal' || key === 'macd_signal') {
    const sig  = Number(value)
    const line = get('MACD Line', 'macd_line')
    if (isNaN(sig) || isNaN(line)) return null
    if (line > sig) return { label: 'Bullish', color: 'success' }
    if (line < sig) return { label: 'Bearish', color: 'warning' }
    return                   { label: 'Neutral', color: 'neutral' }
  }

  // Bollinger Middle — 20-day MA; classify by price position relative to it
  if (key === 'Bollinger Middle' || key === 'bollinger_middle' || key === 'bollinger_bands_middle') {
    const mid = Number(value)
    if (isNaN(mid) || price === 0) return null
    const diffPct = Math.abs((price - mid) / price) * 100
    if (diffPct <= 0.5) return { label: 'Neutral', color: 'neutral' }
    if (price > mid)    return { label: 'Bullish', color: 'success' }
    return                       { label: 'Bearish', color: 'warning' }
  }
...
function Section({ title, icon, children, defaultOpen = false }: {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-iron bg-graphite">
      <button
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between px-5 py-5 hover:bg-[rgba(255,255,255,0.02)] transition-colors duration-150 text-left"
      >
      ...
      {open && (
        <div className="px-4 sm:px-6 pb-4 sm:pb-6 pt-4 sm:pt-5 border-t border-iron animate-fade-in">
          {children}
        </div>
      )}
    </div>
  ...
  return (
    <div className="rounded-xl border border-iron bg-graphite overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-2">
        {/* Entry */}
        <div className="flex items-center justify-between px-3 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderLeft: '3px solid rgba(6,100,232,0.7)' }}>
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.04em] mb-0.5" style={{ color: 'rgba(6,100,232,0.7)' }}>{PRICE_LEVEL_LABELS.entry}</p>
            <p className="font-display text-[22px] font-medium text-white tabular-nums leading-none">
              {entryNum != null ? fmtPrice(entryNum) : analysis.trading_plan.entry_price}
            </p>
          </div>
          {entryVsCurrent != null && (
            <span className="font-sans text-[13px] font-medium tabular-nums" style={{ color: 'rgba(6,100,232,0.7)' }}>
        ...
        {/* Target / Resistance */}
        <div className="flex items-center justify-between px-3 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderLeft: '3px solid rgba(16,185,129,0.7)' }}>
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.04em] mb-0.5" style={{ color: 'rgba(16,185,129,0.7)' }}>{PRICE_LEVEL_LABELS.t1}</p>
            <p className="font-display text-[22px] font-medium text-white tabular-nums leading-none">{fmtPrice(targetNum)}</p>
          </div>
          <span className="font-sans text-[13px] font-medium tabular-nums" style={{ color: '#10b981' }}>
            +{targetGainPct.toFixed(2)}%
          </span>
        </div>

        {/* Stop / Support */}
        <div className="flex items-center justify-between px-3 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderLeft: '3px solid rgba(244,63,94,0.7)' }}>
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.04em] mb-0.5" style={{ color: 'rgba(244,63,94,0.7)' }}>{PRICE_LEVEL_LABELS.stop}</p>
            <p className="font-display text-[22px] font-medium text-white tabular-nums leading-none">
              {stopNum != null
                ? fmtPrice(stopNum)
          ...
          {stopLossPct != null && (
            <span className="font-sans text-[13px] font-medium tabular-nums" style={{ color: '#f43f5e' }}>
          ...
          <div className="pt-1">
            ...
            <div className="flex gap-2 flex-wrap">
              {(['target_2', 'target_3'] as const).map((k, i) => {
                ...
                return tNum ? (
                  <span key={k} className="font-mono text-xs font-medium text-[#10b981] tabular-nums px-2 py-1 rounded-full" style={{ background: 'transparent', border: '1px solid rgba(16,185,129,0.2)' }}>
    ...
    <div className="rounded-xl border border-iron bg-graphite p-4">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="font-mono text-sm font-semibold text-[#10b981] tabular-nums leading-none">+{targetGainPct.toFixed(2)}%</p>
        </div>
        <div className="flex flex-col gap-1">
          <p className="text-[10px] font-semibold text-ash uppercase tracking-[0.04em]">To Support</p>
          <p className="font-mono text-sm font-semibold text-[#f43f5e] tabular-nums leading-none">{stopLossPct.toFixed(2)}%</p>
        </div>
        <div className="flex flex-col gap-1">
          <p className="text-[10px] font-semibold text-ash uppercase tracking-[0.04em]">R:R</p>
          <p className="font-mono text-sm font-semibold text-white tabular-nums leading-none">1:{rrRatio.toFixed(2)}</p>
          <span className="inline-block text-[10px] font-medium mt-0.5 px-[7px] py-[2px] rounded-full" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: '#777a88' }}>
      ...
      <div className="flex items-center rounded-full overflow-hidden h-1 bg-iron">
    ...
    <div key={label} className={cn('rounded-xl border bg-inkwell p-4', paletteBorder[palette] ?? 'border-iron')}>
      ...
      <div className="flex flex-col divide-y divide-iron">
        {items.map(([k, v]) => {
          ...
          return (
            <div key={k} className="flex items-center justify-between gap-2 py-2.5 sm:py-3 first:pt-0 last:pb-0">
              <span className="flex items-center text-[14px] font-normal text-[#acafb9] min-w-0">
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {scalarRows.length > 0 && (
        <div className="flex flex-col divide-y divide-iron">
          {scalarRows.map(([k, v]) => (
            <div key={k} className="flex items-center justify-between py-3 sm:py-4">
...
// ─── Signal Row ───────────────────────────────────────────────────────────────
function SignalRow({ point }: { point: ReasoningPoint }) {
  const [expanded, setExpanded] = useState(false)
  const isLong = point.text.length > 80

  const badgeColor: BadgeColor = point.status === 'bullish' ? 'success'
    : point.status === 'bearish' ? 'danger' : 'warning'
  const badgeLabel = point.status === 'bullish' ? 'Positive'
    : point.status === 'bearish' ? 'Bearish' : 'Caution'

  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-iron last:border-b-0">
      <Badge color={badgeColor} size="sm" className="shrink-0 mt-0.5 font-medium tracking-[0.06em]">
        {badgeLabel}
      </Badge>
      <div className="flex-1 min-w-0">
        <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#777a88] mb-0.5">
          {point.label}
        </p>
        <p className={cn('text-xs text-[#acafb9] leading-relaxed', !expanded && isLong && 'line-clamp-1')}>
          {point.text}
        </p>
      </div>
      {isLong && (
        <button
          ...
          className="shrink-0 mt-0.5 text-ash hover:text-pearl transition-colors duration-100"
        >
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div className="flex items-center gap-3 sm:pr-5 sm:border-r sm:border-iron">
              <div className="flex flex-col gap-0.5">
                ...
                <div className="flex items-baseline gap-1 leading-none">
                  <span className="font-mono text-3xl font-semibold text-brand-blue tabular-nums">
            ...
            <div className="flex flex-col gap-0.5 sm:px-5 sm:border-r sm:border-iron">
              ...
              <div className="flex items-baseline gap-2 leading-none">
                <span className="font-mono text-2xl sm:text-3xl font-medium text-white tabular-nums">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-5 items-stretch">
            ...
            <div className="rounded-xl border border-iron bg-graphite overflow-hidden">
              ...
              <div className="px-4 sm:px-6 py-4 sm:py-5">
                <div className="flex flex-col">
                  {[
                    ...points.filter(p => p.status === 'bullish'),
                    ...points.filter(p => p.status === 'neutral'),
                    ...points.filter(p => p.status === 'bearish'),
                  ].map((pt, i) => (
                    <SignalRow key={i} point={pt} />
                  ))}
                </div>
              </div>
        ...
        <div className="hidden lg:flex flex-col gap-4 lg:sticky lg:top-[4.5rem] self-start">
          <PriceLevelsCard analysis={analysis} />
          <RiskRewardCard analysis={analysis} />
        </div>
      </div>
