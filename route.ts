...
const QUICK_STATS = [
  {
    label: 'Stocks available', value: '240+',
    icon: (
...
export default function DashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-8 animate-fade-in dashboard-container">
        ...
        {QUICK_STATS.map((stat) => (
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11px] text-surface-500 mt-1 uppercase tracking-wide">{stat.label}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {QUICK_ACTIONS.map((action) => (
            <Link
              ...
              style={{ borderLeft: '3px solid rgba(6,100,232,0.3)' }}
            >
              ...
              <svg
                ...
                className="text-brand-blue opacity-60 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all duration-150 shrink-0 ml-auto"
              >
      ...
      >
        <p className="font-sans font-semibold text-sm text-white mb-6">Get started</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">

          {/* Analyze a stock */}
          <div className="flex flex-col gap-1.5">
            <div className="w-9 h-9 rounded-lg bg-brand-blue/10 border border-brand-blue/20 flex items-center justify-center text-brand-blue shrink-0 mb-1">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M3 14l4.5-5.5 3.5 3 5-6.5"/><path d="M3 17h14"/>
              </svg>
            </div>
            <p className="font-sans font-semibold text-[15px] text-white">Analyze a stock</p>
            <p className="text-[13px] leading-relaxed" style={{ color: '#777a88' }}>
              Search any NSE or BSE ticker for instant AI signals, entry zones, and price targets.
            </p>
            <Link href="/stocks" className="text-xs font-medium text-brand-blue hover:text-brand-cyan transition-colors mt-1">
              Go to stocks →
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <p className="font-sans font-semibold text-[15px] text-white">Build a portfolio</p>
            <p className="text-[13px] leading-relaxed" style={{ color: '#777a88' }}>
              Answer 3 questions and get a fully allocated NSE/BSE portfolio with position sizes and targets.
            </p>
            <Link href="/portfolio" className="text-xs font-medium text-brand-blue hover:text-brand-cyan transition-colors mt-1">
              Build now →
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="w-9 h-9 rounded-lg bg-brand-blue/10 border border-brand-blue/20 flex items-center justify-center text-brand-blue shrink-0 mb-1">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M3 12l3.5-4 3 3 3.5-5 3.5 3.5"/>
              </svg>
            </div>
      ...
      {/* ── Recently viewed — only rendered when non-empty ── */}
      {hasRecentlyViewed && (
        <div>
      ...
      <div className="flex items-center gap-4 pt-4 border-t border-gray-100 dark:border-surface-800 flex-wrap">
        ...
        <AIDisclosureNote variant="inline" className="text-[10px] text-rose-400/70 ml-auto" />
      </div>
