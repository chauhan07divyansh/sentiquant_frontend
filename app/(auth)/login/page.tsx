...
// ─────────────────────────────────────────────
//  LOGIN CONTENT
//  Separated from LoginPage so useSearchParams()
//  is inside a Suspense boundary (Next.js 14 req).
// ─────────────────────────────────────────────
function LoginContent() {
  ...
  // UX: separate state for the "forgot password" info message so it uses neutral styling
  ...
  // AUTH: Resolve the post-login destination.
  // Reads `?from` (set by manual app redirects) or `?callbackUrl` (set by NextAuth
  // when withAuth blocks an unauthenticated request). Falls back to /dashboard.
  // Absolute callbackUrl values (e.g. http://host/stocks) are reduced to pathname.
  ...
  // AUTH: Already-authenticated users should never see the login form
  useEffect(() => {
    if (isAuthenticated) router.replace(getReturnUrl())
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAuthenticated])

  // Show error from URL param (e.g. session expired)
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // AUTH: Success — send to return URL or dashboard
  ...
  if (sessionLoading) return null   // Avoid flash before session resolves
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      {/* Demo hint */}
      {/* FIXED: teal-300 (#1affbd) fails WCAG on white — replaced with #00967a in light mode */}
      <div className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl bg-brand-blue/8 border border-brand-cyan/20 text-xs text-brand-blue dark:text-brand-cyan leading-relaxed">
        ...
        <span>
          Demo credentials: <span className="font-mono font-semibold">demo@sentiquant.com</span> /{' '}
      ...
      >
        {/* Global error — rose styling for real auth failures */}
        {formError && (
          <div
            role="alert"
            className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl bg-rose-400/8 border border-rose-400/20 text-xs text-rose-300 leading-relaxed"
          >
        ...
        {/* UX: neutral info message for "forgot password" — not an error, so no rose styling */}
        {forgotInfo && (
          <div
            role="status"
            className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl bg-surface-800/50 border border-surface-700 text-xs text-surface-400 leading-relaxed"
          >
        ...
        {/* Forgot password (placeholder — no backend endpoint yet) */}
        <div className="flex justify-end -mt-1">
          <button
            type="button"
            className="text-xs text-surface-500 hover:text-brand-cyan transition-colors"
            // UX: shows a neutral info message instead of the error-styled formError
...
// ─────────────────────────────────────────────
//  LOGIN PAGE
//  Wraps LoginContent in Suspense so useSearchParams()
