          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-gray-100 dark:border-surface-800 bg-gray-50/50 dark:bg-surface-900/50">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-surface-500 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('symbol')}>Stock <SortIcon k="symbol" /></th>
                <th className="text-right px-4 py-3 text-[10px] font-semibold text-surface-500 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('allocation')}>Allocation <SortIcon k="allocation" /></th>
                ...
                <th className="text-right px-4 py-3 text-[10px] font-semibold text-surface-500 uppercase tracking-wider">Technical Support</th>
                <th className="text-right px-4 py-3 text-[10px] font-semibold text-surface-500 uppercase tracking-wider">Risk ₹</th>
                <th className="text-right px-4 py-3 text-[10px] font-semibold text-surface-500 uppercase tracking-wider">R1</th>
                <th className="text-right px-4 py-3 text-[10px] font-semibold text-surface-500 uppercase tracking-wider">R2</th>
                <th className="text-right px-4 py-3 text-[10px] font-semibold text-surface-500 uppercase tracking-wider">R3</th>
                <th className="text-right px-4 py-3 text-[10px] font-semibold text-surface-500 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('score')}>Score <SortIcon k="score" /></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 dark:divide-surface-800/60">
              {sorted.map((h, i) => {
                ...
                return (
                  <tr key={`${h.symbol}-${i}`} className="hover:bg-gray-50/50 dark:hover:bg-surface-800/20 transition-colors animate-fade-in" style={{ animationDelay: `${i * 20}ms`, animationFillMode: 'both' }}>
                    ...
                    <td className="px-4 py-3 text-right font-mono font-semibold text-surface-900 dark:text-white tabular-nums text-sm">{h.investment_amount ? formatINRCompact(h.investment_amount) : '—'}</td>
                    ...
                    <td className="px-4 py-3 text-right font-mono text-emerald-400/80 tabular-nums text-[11px]">{(h as HoldingWithTargets).target_1 ? formatINR((h as HoldingWithTargets).target_1!, 0) : '—'}</td>
                    ...
                    <td className="px-4 py-3 text-right"><ScoreBadge score={h.score} /></td>
                  </tr>
                )
              })}
            </tbody>
              ...
              <div key={`${h.symbol}-${i}`} className="p-4 animate-fade-in" style={{ animationDelay: `${i * 20}ms`, animationFillMode: 'both' }}>
                ...
                <div className="grid grid-cols-3 gap-2 pl-6">
                  {[
                    ...
                    { label: 'Support',   value: h.stop_loss ? formatINR(h.stop_loss, 0) : '—',                     color: 'text-rose-400' },
                    ...
                    { label: 'R1',        value: (h as HoldingWithTargets).target_1 ? formatINR((h as HoldingWithTargets).target_1!, 0) : '—', color: 'text-emerald-400' },
                    { label: 'R2',        value: (h as HoldingWithTargets).target_2 ? formatINR((h as HoldingWithTargets).target_2!, 0) : '—', color: 'text-emerald-400' },
                  ].map(m => (
                    <div key={m.label}>
  ...
  return (
    <div className="flex flex-col gap-8 dashboard-container-narrow">
      <div className="flex flex-col gap-2">
        ...
        <div style={{ width: '64px', height: '1px', background: 'rgba(255,255,255,0.06)', margin: '16px auto 0' }} />
      </div>
        ...
        isGenerating ? (
          ...
          <div className="hero-entry-4 flex flex-col gap-6">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
              ...
              <div style={{ filter: 'blur(2px)', pointerEvents: 'none', userSelect: 'none', display: 'flex', gap: '12px', width: '100%', overflowX: 'auto', paddingBottom: '2px' }}>
                {/* Card 1 — AI Grade */}
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '16px', minWidth: '160px', flex: '1' }}>
                  <p className="font-display" style={{ fontSize: '36px', color: '#0664e8', lineHeight: 1, margin: '0 0 6px' }}>A+</p>
                  <p style={{ fontFamily: 'var(--font-inter), Inter, sans-serif', fontSize: '11px', color: '#777a88', margin: 0 }}>Portfolio grade</p>
                </div>
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '16px', minWidth: '160px', flex: '1' }}>
                  ...
                  <p style={{ fontFamily: 'var(--font-inter), Inter, sans-serif', fontSize: '11px', color: '#777a88', margin: 0 }}>Top holdings</p>
                </div>
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '16px', minWidth: '160px', flex: '1' }}>
                  ...
                  <p style={{ fontFamily: 'var(--font-inter), Inter, sans-serif', fontSize: '11px', color: '#777a88', margin: 0 }}>Diversification score</p>
                </div>
              </div>
            </div>
            ...
            <div key={step} className={direction === 'forward' ? 'animate-step-in' : 'animate-step-back'}>
              ...
              {/* ── Step 1 — Strategy ── */}
              {step === 1 && (
                <div className="flex flex-col gap-4 rounded-2xl p-5 sm:p-6" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="relative hidden sm:flex flex-col gap-3 p-5 rounded-xl border border-gray-200 dark:border-surface-800 bg-surface-900/30 cursor-not-allowed opacity-60 select-none">
                      {/* Coming soon badge */}
                      ...
                      <div>
                        ...
                        <p className="text-xs text-surface-600 mt-1 leading-relaxed">Long-term holds · 6 months – 5 years · launching soon</p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-center gap-6 flex-wrap pt-1">
                    {([
                      ...
                      <span key={label} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#5e616e', fontFamily: 'var(--font-inter), Inter, sans-serif' }}>
                        {icon}{label}
                      </span>
                    ))}
                  </div>
              ...
              {/* ── Step 3 — Budget ── */}
              {step === 3 && (
                <div className="flex flex-col gap-5 rounded-2xl p-5 sm:p-6" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <div>
