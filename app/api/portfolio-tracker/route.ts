...
async function readData(): Promise<PortfolioData> {
  ...
  const json = JSON.parse(raw) as Partial<PortfolioData>
  return {
...
export async function GET() {
  try {
    const { trackingEnabled, trackingStartDate, positions } = await readData()
    ...
    return NextResponse.json(
      { success: true, trackingEnabled, trackingStartDate, positions: withLive },
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (typeof body.trackingEnabled === 'boolean') data.trackingEnabled = body.trackingEnabled
    if ('trackingStartDate' in body) {
      data.trackingStartDate = body.trackingStartDate ? String(body.trackingStartDate) : null
    }
    await writeData(data)
    return NextResponse.json({ success: true, trackingEnabled: data.trackingEnabled, trackingStartDate: data.trackingStartDate })
  } catch (e) {
    console.error('[portfolio-tracker] PATCH error:', e)
    return NextResponse.json({ success: false, error: 'Failed to update tracking config' }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const newPosition: TrackedPosition = {
      ...
      allocatedAmount:
        body.allocatedAmount != null && body.allocatedAmount !== ''
          ? Number(body.allocatedAmount)
          : null,
      riskAppetite:
        body.riskAppetite === 'low' || body.riskAppetite === 'medium' || body.riskAppetite === 'high'
          ? body.riskAppetite
          : null,
    }
      ...
      allocatedAmount:
        body.allocatedAmount !== undefined
          ? body.allocatedAmount != null && body.allocatedAmount !== ''
            ? Number(body.allocatedAmount)
            : null
          : (prev.allocatedAmount ?? null),
      riskAppetite:
        body.riskAppetite !== undefined
          ? body.riskAppetite === 'low' || body.riskAppetite === 'medium' || body.riskAppetite === 'high'
            ? body.riskAppetite
            : null
          : (prev.riskAppetite ?? null),
    }
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const data     = await readData()
    const filtered = data.positions.filter((p) => p.id !== id)
