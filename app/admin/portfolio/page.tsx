...
export default function AdminPortfolioPage() {
  ...
  const [patchingTracking, setPatchingTracking] = useState(false)
  ...
  const loadPositions = useCallback(async (secret?: string) => {
    ...
    try {
      ...
      setTrackingStartDate(json.trackingStartDate ?? '')
    } catch {
  ...
  function openEdit(pos: TrackedPositionWithLive) {
    ...
    setFormData({
      ...
      notes:           pos.notes,
      allocatedAmount: pos.allocatedAmount != null ? String(pos.allocatedAmount) : '',
      riskAppetite:    pos.riskAppetite === 'low' || pos.riskAppetite === 'high' ? pos.riskAppetite : 'medium',
    })
    setFormError('')
    setShowForm(true)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const body = {
        ...
        allocatedAmount: formData.allocatedAmount ? Number(formData.allocatedAmount) : null,
  ...
  async function handlePatchTracking(updates: { trackingEnabled?: boolean; trackingStartDate?: string | null }) {
    setPatchingTracking(true)
    try {
      const res = await fetch('/api/portfolio-tracker', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'x-admin-secret': getSecret() },
      ...
      const json = (await res.json()) as { trackingEnabled?: boolean; trackingStartDate?: string | null }
      setTrackingEnabled(json.trackingEnabled ?? false)
      setTrackingStartDate(json.trackingStartDate ?? '')
    } finally {
      setPatchingTracking(false)
    }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#000000', padding: '48px 24px' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          >
            ...
            <form onSubmit={handleSubmit}>
              ...
              >
                ...
                <label style={labelStyle}>
                  <span style={labelTextStyle}>Allocated Amount (₹)</span>
                  <input
                    className="input-base"
                    type="number"
                    step="1"
                    ...
                    onChange={(e) => setField('allocatedAmount', e.target.value)}
                  />
                </label>
                <label style={labelStyle}>
                  <span style={labelTextStyle}>Risk Appetite</span>
                  <select
                    className="input-base"
                    value={formData.riskAppetite}
                    onChange={(e) => setField('riskAppetite', e.target.value as 'low' | 'medium' | 'high')}
                  >
        ...
        {/* Tracking Control */}
        <div style={{ background: '#0a0a0a', border: '1px solid #2e3038', borderRadius: '10px', padding: '24px 28px', marginBottom: '28px' }}>
          <h2 style={{ fontFamily: 'var(--font-inter), Inter, system-ui, sans-serif', fontSize: '13px', fontWeight: 600, color: '#acafb9', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 20px' }}>
            Tracking Settings
          </h2>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: '20px' }}>
            {/* Enable / Disable toggle */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <span style={{ fontFamily: 'var(--font-inter)', fontSize: '11px', fontWeight: 500, color: '#5e616e', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                Status
              </span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <span style={{ fontFamily: 'var(--font-inter)', fontSize: '13px', color: trackingEnabled ? '#10b981' : '#f43f5e', fontWeight: 600 }}>
                  {trackingEnabled ? 'Enabled' : 'Disabled'}
                </span>
                <button
                  onClick={() => handlePatchTracking({ trackingEnabled: !trackingEnabled })}
                  disabled={patchingTracking}
                  style={{
                    background: trackingEnabled ? 'rgba(244,63,94,0.08)' : 'rgba(16,185,129,0.08)',
                    border: `1px solid ${trackingEnabled ? 'rgba(244,63,94,0.3)' : 'rgba(16,185,129,0.3)'}`,
                    borderRadius: '2px',
                    color: trackingEnabled ? '#f43f5e' : '#10b981',
                    fontFamily: 'var(--font-inter), Inter, system-ui, sans-serif',
                    fontSize: '12px',
                    fontWeight: 500,
                    padding: '6px 16px',
                    cursor: patchingTracking ? 'not-allowed' : 'pointer',
                    opacity: patchingTracking ? 0.6 : 1,
                  }}
                >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                />
                <button
                  onClick={() => handlePatchTracking({ trackingStartDate: trackingStartDate || null })}
                  disabled={patchingTracking}
                  style={{
                    background: 'none',
                    border: '1px solid #2e3038',
