    ...
    <div className="flex flex-col gap-6 dashboard-container-narrow">
      ...
      <div key={tab} className="animate-slide-in-right">
        {isPending ? (
          ...
          // Guest limit exceeded — show signup prompt
          (error as Error).message?.includes('Guest limit') ? (
            <div className="flex flex-col items-center gap-4 py-16 text-center">
              <p className="text-lg font-semibold text-white">You&apos;ve used your 1 free guest analysis</p>
              <p className="text-sm text-surface-400">Sign up free for 10 analyses every day</p>
              <div className="flex gap-3">
                <button
                  onClick={() => signIn('google', { callbackUrl: `/stocks/${symbol}` })}
                  className="flex items-center gap-2.5 py-2.5 px-5 rounded-xl bg-white text-gray-800 font-semibold text-sm hover:bg-gray-50 transition-all shadow-md"
                >
                  <svg width="18" height="18" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                ...
                <Link href="/signup" className="flex items-center px-5 py-2.5 rounded-xl border border-surface-700 text-white font-medium text-sm hover:bg-surface-800 transition-all">
                  Sign up free
                </Link>
              </div>
            </div>
