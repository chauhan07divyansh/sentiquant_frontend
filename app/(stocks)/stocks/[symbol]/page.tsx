...
export default function StockDetailPage({ params }: { params: { symbol: string } }) {
  const decodedRaw      = decodeURIComponent(params.symbol)
  const isInvalidSymbol = !decodedRaw.trim() ||
    ['ANY-SYMBOL', 'UNDEFINED', 'NULL'].some(p => decodedRaw.toUpperCase().includes(p))
  const symbol = decodedRaw.toUpperCase()
  ...
  // Guest view counter — same key as TickerStrip; stocks page is the single setter
  const [guestLimitReached, setGuestLimitReached] = useState(false)

  useEffect(() => {
    if (authStatus !== 'unauthenticated') return
    const count = parseInt(localStorage.getItem('sq_guest_views') ?? '0', 10)
    if (count >= 1) setGuestLimitReached(true)
  }, [authStatus])

  useEffect(() => { addRecentlyViewed(symbol) }, [symbol]) // eslint-disable-line react-hooks/exhaustive-deps

  // Guest query — only runs when not logged in, on swing tab, and within the free view
  const guestQuery = useQuery({
    queryKey:  ['guest-analysis', symbol],
    queryFn:   () => fetchGuestAnalysis(symbol),
    enabled:   !isInvalidSymbol && isGuest && tab === 'swing' && !guestLimitReached,
    retry:     false,
    staleTime: 5 * 60 * 1000,
  })

  // Mark the one free view as used once analysis loads successfully
  useEffect(() => {
    if (guestQuery.isSuccess) localStorage.setItem('sq_guest_views', '1')
  }, [guestQuery.isSuccess])

  // Authenticated queries
  const swingQuery    = useSwingAnalysis(symbol,     { enabled: !isInvalidSymbol && isLoggedIn && tab === 'swing'    })
  const positionQuery = usePositionAnalysis(symbol,  { enabled: !isInvalidSymbol && isLoggedIn && tab === 'position' })
  const compareQuery  = useCompareStrategies(symbol, { enabled: !isInvalidSymbol && isLoggedIn && tab === 'compare'  })
  ...
  const tabs = [
    { id: 'swing',    label: 'Swing',    sub: '1–4 weeks'    },
    { id: 'position', label: 'Position', sub: '6–18 months'  },
  ...
  // ── Back link shared between early-return states ──
  const backLink = (
    <Link
      href="/stocks"
      ...
      <div className="flex flex-col gap-6 dashboard-container-narrow">
        ...
        <div className="flex flex-col items-center gap-5 py-20 text-center">
          <div className="w-16 h-16 rounded-2xl bg-surface-800 border border-surface-700 flex items-center justify-center">
            <svg width="28" height="28" viewBox="0 0 28 28" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" className="text-surface-400">
              <circle cx="12" cy="12" r="9"/><path d="M21 21l-4.35-4.35M15.5 9.5a4 4 0 11-5.66 5.66A4 4 0 0115.5 9.5z"/>
            </svg>
          </div>
          <div className="flex flex-col gap-2">
            <p className="text-lg font-medium text-white">Stock not found</p>
            <p className="text-sm text-surface-400 max-w-xs leading-relaxed">
              We couldn&apos;t find a stock matching that symbol. Check the spelling or browse all stocks.
            </p>
          </div>
          <Link
            href="/stocks"
            className="px-5 py-2.5 rounded-xl border border-surface-700 text-sm font-medium text-white hover:bg-surface-800 transition-colors"
          >
            Browse all stocks
          </Link>
        </div>
  ...
  // ── Guard: API error (authenticated users only — guests keep their own error flow) ──
  if (!isGuest && !isPending && error) {
    return (
      <div className="flex flex-col gap-6 dashboard-container-narrow">
        {backLink}
        <ErrorState
  ...
  return (
    <div className="flex flex-col gap-6 dashboard-container-narrow">
      ...
      {/* Hard gate for guests who have already used their one free view */}
      {guestLimitReached && <GuestGateModal />}

      {/* ── Back link ── */}
      <Link
        href="/stocks"
        className="inline-flex items-center gap-1.5 text-xs text-surface-500 hover:text-surface-200 transition-colors self-start py-3"
      >
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          <path d="M7.5 2.5L4 6l3.5 3.5"/>
        </svg>
      ...
      {isGuest && (
        <div className="flex items-center justify-between gap-3 px-4 py-2.5 rounded-xl bg-brand-blue/10 border border-brand-blue/20 text-sm">
          ...
          <Link href="/signup" className="shrink-0 text-xs font-semibold text-brand-cyan hover:underline py-2.5 px-3">
            Sign up free →
          </Link>
        </div>
      )}
        ...
        <div className="flex items-center gap-3">
          <div>
            ...
            {(data as StockAnalysis)?.company_name && (
              <p className="hero-entry-2 text-[13px] text-[#777a88] mt-0.5">{(data as StockAnalysis).company_name}</p>
            )}
          </div>
        </div>
        <button
      ...
      <div className="hero-entry-4 relative grid grid-cols-3 p-1 bg-surface-900 border border-surface-800 rounded-xl self-start">
        ...
        {tabs.map((t) => (
          ...
          >
            ...
            <span className={cn('text-[12px]', tab === t.id ? 'text-white/70' : 'text-surface-500', t.id === 'compare' && 'hidden sm:inline')}>{t.sub}</span>
          </button>
        ))}
      </div>
      ...
      <div key={tab} className="animate-slide-in-right">
        {isPending ? (
          ...
          <ErrorBoundary context="StockAnalysisView">
            <AnalysisView analysis={data as StockAnalysis} onSwitchTab={setTab} />
          </ErrorBoundary>
        )}
      </div>

      {/* ── Legal disclaimer ── */}
      <AIDisclosureNote variant="compact" className="text-[10px] text-surface-700 pb-2" />
