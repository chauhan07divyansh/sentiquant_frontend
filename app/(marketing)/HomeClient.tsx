      ...
      <div className="relative card rounded-xl overflow-hidden p-5 flex flex-col gap-4 hero-entry-mockup animate-float" style={{ boxShadow: '0 48px 120px -12px rgba(0,0,0,0.9), 0 16px 48px -8px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.07), 0 0 80px 20px rgba(6,100,232,0.25)' }}>
        ...
        <div className="flex items-start justify-between gap-3 pt-1">
          ...
          <div className="flex flex-col items-center justify-center w-12 h-12 rounded-xl bg-emerald-500/10 border border-emerald-500/20 shrink-0">
            ...
            <span className="text-[10px] text-emerald-400/70 uppercase tracking-wide mt-0.5">score</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[
            ...
            <div key={t.label} className="flex flex-col gap-0.5 rounded-lg px-2 py-2 bg-surface-800/40 border border-surface-700/30">
              <span className="text-[10px] text-surface-500 uppercase tracking-wider font-medium">{t.label}</span>
...
function TickerStrip() {
  ...
  useEffect(() => {
    ...
    function onTouchMove(e: TouchEvent) {
      ...
      // Decide swipe axis on first detectable movement
      if (!isHorizSwipeRef.current && (Math.abs(dx) > 3 || Math.abs(dy) > 3)) {
        isHorizSwipeRef.current = Math.abs(dx) >= Math.abs(dy)
      }
      if (!isHorizSwipeRef.current) return
      e.preventDefault() // stop page scroll while swiping the ticker
      const newX = dragBaseOffsetRef.current + dx
      if (trackRef.current) {
  ...
  function resumeFromCurrentPosition() {
    ...
    const rawX = m ? parseFloat(m[1]) : 0
    // Normalise into the loop range [−halfWidth, 0]
    const loopX = halfWidth > 0 ? Math.max(-halfWidth, Math.min(0, rawX % halfWidth)) : 0
    const progress = halfWidth > 0 ? -loopX / halfWidth : 0 // 0..1
    const delay = -(progress * 40) // negative delay = start part-way through the 40s cycle
    el.style.transform = ''
  ...
  function handleMouseDown(e: React.MouseEvent<HTMLDivElement>) {
    ...
    dragBaseOffsetRef.current = getTrackOffset()
  ...
  function handleMouseMove(e: React.MouseEvent<HTMLDivElement>) {
    ...
    const newX = dragBaseOffsetRef.current + delta
    if (trackRef.current) {
      trackRef.current.style.transform = `translateX(${newX}px)`
    }
  }
  ...
  return (
    <>
      ...
      >
        ...
        <div
          ref={trackRef}
          className="ticker-x-track"
          onClickCapture={(e) => {
            // Suppress click on children when the interaction was a drag, not a tap
            if (dragDistanceRef.current > 5) e.stopPropagation()
          }}
        >
          {doubled.map((stock, i) => (
            <button
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px', padding: '14px 24px' }}>
                {[
                  ...
                  <div key={t.label} style={{ display: 'flex', flexDirection: 'column', gap: '2px', padding: '8px', background: '#1c1d22', borderRadius: '6px' }}>
                    <span style={{ fontSize: '10px', color: '#777a88', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-inter)' }}>{t.label}</span>
    ...
    <div className="flex flex-col [overflow-x:clip]">
      <style>{`
        @media (max-width: 639px) {
          ...
          .hero-cta-row a, .hero-cta-row button { width: 100% !important; box-sizing: border-box !important; justify-content: center !important; }
        }
            ...
            <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
              ...
              >
                ...
                {status === 'loading' ? (
                  <div
                    aria-hidden="true"
                    className="inline-flex border border-[#2e3038] rounded-[2px]"
                    style={{ padding: '11px 24px', width: '190px', opacity: 0.35 }}
                  />
                  ...
                  <button
                    ...
                    onClick={() => {
                      setIsSigningIn(true)
                      track.guestSignupClicked('hero_cta', 'google')
                      signIn('google', { callbackUrl: '/dashboard' })
                    }}
                    className="inline-flex items-center gap-3 border border-[#2e3038] rounded-[2px] bg-transparent text-[14px] font-medium hover:border-[#464853] hover:text-white transition-colors duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
                    style={{ padding: '11px 24px', color: '#acafb9', fontFamily: 'var(--font-inter)' }}
                  >
                    {isSigningIn ? (
