...
const TESTIMONIALS = [
  {
    quote: "I used to spend 3 hours analyzing a single stock. Now I get better analysis in 60 seconds. Sentiquant is the edge I didn't know I was missing.",
    name: 'Rahul M.',
    role: 'Swing trader, Mumbai',
    accent: 'cyan',
  },
  {
    quote: "The AI scoring system is spot on. I ran it on my existing portfolio and it flagged two underperformers I had been holding out of habit. Sold both.",
    name: 'Priya S.',
    role: 'Long-term investor, Bangalore',
    accent: 'blue',
  },
  {
    quote: "Finally a tool built for Indian markets. The NSE stock signals with proper stop-losses and targets are exactly what I needed as a part-time trader.",
    name: 'Arjun K.',
    role: 'Part-time trader, Hyderabad',
    accent: 'cyan',
  },
  {
    quote: "The portfolio generator saved me hours of allocation math. I entered my budget, set my risk level, and walked away with a full plan in minutes.",
    name: 'Sneha R.',
...
export function HomeClient() {
  ...
  const { ref: socialRef,       inView: socialInView       } = useInView<HTMLDivElement>(0.05)
  ...
  // ANIMATION: Counter hooks for the three numeric stats
  const counter1 = useCountUp(5000, 2000) // "5,000+"
  ...
  return (
    <div className="flex flex-col overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            <div className="flex flex-col items-center lg:items-start gap-7 text-center lg:text-left">
              ...
              <p className="hero-entry-5 text-xs text-surface-600">
                No credit card required · Trusted by 5,000+ traders
              </p>
            </div>
      ...
      <section className="relative border-y border-surface-800 bg-surface-900/40 overflow-hidden">
        ...
        <div
          ref={statsRef}
          className="relative max-w-5xl mx-auto px-4 sm:px-6 py-12 grid grid-cols-2 sm:grid-cols-4 gap-8"
        >
          {/* ANIMATION: counter-animated stats — first 3 count up, last is static */}
          {[
            { display: statsInView ? `${counter1.count.toLocaleString()}+` : '5,000+', label: 'Traders trust Sentiquant' },
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-20 flex flex-col gap-12">
        ...
        {/* ANIMATION: section header fades in */}
        <div className={cn('text-center flex flex-col gap-3 scroll-reveal', socialInView && 'in-view')}>
          <span className="text-xs font-semibold text-brand-cyan uppercase tracking-widest">
            Trusted by traders
          </span>
          <h2 className="font-display text-3xl sm:text-4xl font-bold text-white tracking-tight">
            5,000+ traders use Sentiquant daily
          </h2>
          <p className="text-sm text-surface-400 max-w-md mx-auto leading-relaxed">
            From first-time investors to active swing traders across India.
          </p>
        </div>

        {/* ANIMATION: testimonial cards stagger in — ref on the grid wrapper */}
        <div ref={socialRef} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {TESTIMONIALS.map((t, i) => {
            const ac = accentClass(t.accent)
            return (
              // ANIMATION: each card gets a stagger delay via Tailwind `delay-*`
              <div
                key={t.name}
                className={cn(
                  'relative rounded-2xl border border-surface-800 bg-gradient-to-b from-surface-900 to-surface-950 p-6 flex flex-col gap-4 transition-all duration-300 hover:-translate-y-1',
                  'scroll-reveal', socialInView && 'in-view',
                  STAGGER[i],
                  ac.border, ac.glow
                )}
              >
                <div className={`w-8 h-0.5 rounded-full ${ac.quoteBar} opacity-60`} />
      ...
      <section className="border-t border-surface-800 bg-surface-900/20 py-16 px-4 sm:px-6">
        ...
        <div ref={seoRef} className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          <div className={cn('flex flex-col gap-5 scroll-reveal', seoInView && 'in-view')}>
            ...
            <p className="text-sm text-surface-400 leading-relaxed">
              Whether you&apos;re looking for the <strong className="text-surface-300">best stocks to buy in India in 2026</strong>, analysing an intraday setup, or building a long-term portfolio — Sentiquant gives you the signal, the grade, and the reasoning behind it. Trusted by over 5,000 active traders across Mumbai, Bangalore, Hyderabad, and beyond.
            </p>
            <Link
              href="/analysis"
