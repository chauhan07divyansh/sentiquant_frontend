...
// ─────────────────────────────────────────────
//  STATIC PARAMS
//  Generates paths for all posts (file-based + legacy).
// ─────────────────────────────────────────────
export function generateStaticParams() {
  const fileSlugs   = getAllBlogs().map((p) => p.slug)
  const legacySlugs = MOCK_POSTS.map((p) => p.slug)
  const unique = Array.from(new Set([...fileSlugs, ...legacySlugs]))
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const post = getBlogBySlug(params.slug) ?? getPostBySlug(params.slug)
  if (!post) return {}
  return {
    title:       post.title,
...
// ─────────────────────────────────────────────
//  TOC HELPERS — work for both HTML and Markdown
// ─────────────────────────────────────────────
function extractTOC(content: string, isMarkdown: boolean): Array<{ id: string; text: string }> {
...
export default function BlogPostPage({ params }: Props) {
  // File-based markdown posts take priority
  const filePost   = getBlogBySlug(params.slug)
  const legacyPost = getPostBySlug(params.slug)
  const post: (BlogPost & { contentType?: 'markdown' | 'html' }) | undefined =
    filePost ?? (legacyPost ? { ...legacyPost, contentType: 'html' } : undefined)

  if (!post) notFound()

  const isMarkdown = post.contentType === 'markdown'
  const toc = post.content ? extractTOC(post.content, isMarkdown) : []

  // All posts for related articles
  const allPosts: BlogPost[] = [
    ...getAllBlogs(),
    ...MOCK_POSTS.filter((p) => !getAllBlogs().some((f) => f.slug === p.slug)),
  ]
          ...
          <div className="min-w-0">
            {post.content ? (
              ...
              <div className="flex flex-col gap-4">
                <p className="prose-custom text-base text-surface-200 leading-relaxed font-medium" itemProp="articleBody">
                  {post.excerpt}
                </p>
