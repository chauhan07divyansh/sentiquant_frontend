...
function CentralPlanet({ isMobile }: { isMobile: boolean }) {
  ...
  return (
    <group ref={groupRef}>
      <mesh>
        <sphereGeometry args={[isMobile ? 0.85 : 1.1, 48, 48]} />
      ...
      <mesh scale={1.03}>
        <sphereGeometry args={[isMobile ? 0.85 : 1.1, 32, 32]} />
        <meshBasicMaterial
          color="#8fc4ff"
          transparent
          opacity={0.18}
...
───────────────────────────────────────────── */
function OrbitBody({
  radius, speed, size, label, currentP, appearAt,
}: {
  radius: number
  speed: number
  size: number
    ...
    <group rotation={[inclination, 0, 0]}>
      <group ref={orbitGroupRef}>
        ...
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          <ringGeometry args={[radius - 0.003, radius + 0.003, 120]} />
          <meshBasicMaterial
            color="#5b9eef"
            transparent
            opacity={0.35}
        ...
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          <ringGeometry args={[radius - 0.01, radius + 0.01, 120]} />
          <meshBasicMaterial
            color="#2a5da8"
            transparent
            opacity={0.08}
