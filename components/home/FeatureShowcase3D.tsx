...
// Original CSS preserve-3d carousel — five product feature cards on a
// rotating cylinder. The active card faces front; side cards are visible
// at 72° offsets, creating the blurred depth effect from the screenshot.
...
const STEP   = 360 / N   // 72° between adjacent cards
const RADIUS = 290        // translateZ distance in px
...
export function FeatureShowcase3D() {
  ...
  // Auto-advance every 4 s; halted under prefers-reduced-motion
  useEffect(() => {
  ...
  return (
    <div ref={ref} className="border-t border-surface-800 py-20 sm:py-24 px-4 sm:px-6 overflow-hidden">
      <div className="max-w-6xl mx-auto flex flex-col gap-10">
        ...
        >
          {/* Spinner — rotateY brings the active card to front */}
          <div
          ...
          >
            {FEATURES.map((feat, i) => {
              ...
              return (
                ...
                >
                  <div className={cn(
                    'relative card rounded-xl overflow-hidden p-5 flex flex-col gap-3 transition-all duration-500 text-left',
                    isFront
                      ? 'border-brand-cyan/30 shadow-[0_0_40px_rgba(6,182,212,0.10)]'
                      : 'opacity-50 border-surface-800/60'
                  )}>
                    {/* Top accent — only on active card */}
                    <div className={cn(
