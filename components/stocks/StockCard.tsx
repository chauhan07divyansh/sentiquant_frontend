...
export const StockListCard = memo(function StockListCard({ symbol, name, sector, className }: StockListItemProps) {
  // PERF: 3D tilt only on desktop — no event listeners on mobile
    ...
    >
      <div
        ...
        className={cn(
          ...
          // PERF: removed will-change-transform from all cards
          // only apply on hover via group — saves GPU memory on mobile
...
export const StockListRow = memo(function StockListRow({ symbol, name, sector }: StockListItemProps) {
  return (
    ...
    >
      ...
      <span className="text-[10px] font-mono text-surface-500 uppercase tracking-wider hidden sm:block shrink-0">NSE</span>
      <svg
